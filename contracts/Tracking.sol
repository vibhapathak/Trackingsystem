...
contract Tracking {
    ...
    struct Shipment {
        address sender;
        address receiver; // Corrected typo from 'reciever' to 'receiver'
        uint256 pickupTime;
    ...
    TypeShipment[] public typeShipments; // Made public for easier interaction
    ...
    function completeShipment(address _sender, address _receiver, uint256 _index) public {
        ...
        payable(shipment.sender).transfer(amount); // Corrected 'payabe' to 'payable'
    ...
    function getShipmentsCount(address _sender) public view returns (uint256) {
        return shipments[_sender].length; // Corrected 'sipments' to 'shipments'
    }

    function getAllTransactions() public view returns (TypeShipment[] memory) {
        return typeShipments; // Corrected 'TypeSipment' to 'TypeShipment'
    }
}
